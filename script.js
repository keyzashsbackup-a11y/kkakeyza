...
//api nya question
const GROQ_API_KEY = "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";
...
const activitiesData = [
  { maxBudget: 30000,   icon: "", title: "Jalan Sore ke Taman",     desc: "Jalan-jalan sore di taman kota. Nikmati udara segar dan obrolan santai.", tag: "Gratis - Rp 30.000"  },
  { maxBudget: 50000,   icon: "", title: "Picnic Sederhana",         desc: "Gelar tikar di taman, bawa makanan ringan buatan sendiri.", tag: "≤ Rp 50.000"         },
  { maxBudget: 100000,  icon: "", title: "Cafe Date",                 desc: "Kunjungi cafe aesthetic terdekat, pesan minuman favorit dan ngobrol berdua.", tag: "≤ Rp 100.000"        },
  { maxBudget: 150000,  icon: "", title: "Nonton Bioskop",           desc: "Nonton film bareng — kencan klasik yang selalu menyenangkan.", tag: "≤ Rp 150.000"        },
  { maxBudget: 200000,  icon: "", title: "Makan Malam Romantis",     desc: "Kunjungi restoran spesial, atau lebih romantis lagi — masak bareng di rumah!", tag: "≤ Rp 200.000"        },
  { maxBudget: 300000,  icon: "", title: "Day Trip ke Alam",        desc: "Pergi ke wisata alam terdekat — pantai, air terjun, atau kebun teh.", tag: "≤ Rp 300.000"        },
  { maxBudget: 500000,  icon: "", title: "Staycation Mini",          desc: "Cari hotel budget yang cozy. Nikmati kolam renang dan sarapan bersama.", tag: "≤ Rp 500.000"        },
  { maxBudget: Infinity, icon: "", title: "Short Vacation",          desc: "Rencanakan liburan singkat ke kota lain — naik kereta atau bus, menginap 1-2 malam.", tag: "Rp 500.000+"         },
];
  ...
  resultArea.innerHTML = `
    ...
    ${placesData.map(p => `
      <div class="result-card">
        ...
        <a href="https://www.google.com/maps/search/${encodeURIComponent(p.mapsQuery + ' ' + city)}" target="_blank" class="btn-maps"> Buka Google Maps</a>
      </div>
    `).join('')}
  `;
}
      ...
      resultArea.innerHTML = `
        ...
        ${placesData.map(p => `
          <div class="result-card">
            ...
            <div class="rc-desc">${p.desc}</div>
            <a href="https://www.google.com/maps/search/${encodeURIComponent(p.mapsQuery)}/@${lat},${lon},14z" target="_blank" class="btn-maps"> Buka Google Maps</a>
          </div>
        `).join('')}
      `;
    },
